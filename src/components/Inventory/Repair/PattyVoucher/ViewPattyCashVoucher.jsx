...
import styles from "./ViewPattyCashVoucher.module.css";
import logo from "../../../../assets/KM-LOGO.png";
import axios from "axios";
import { toWords } from "number-to-words";
import Paid from "../../../../assets/uploads/pay.png"
const API_URL = process.env.REACT_APP_API_URL;
const ViewPattyCashVoucher = ({ voucherId, onBack  }) => {
 ...
 useEffect(() => {
    ...
    const amountStr = data.total_paid.toString();
  ...
  useEffect(() => {
    if (!voucherId) return;
    axios
      .get(`${API_URL}/GetPettyCash/${voucherId}`)
      .then((res) => {
        setData(res.data);
        console.log(res.data); // log the response directly
      })
      .catch((err) => {
        console.log(err);
      });
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.logoContainer}>
        ...
        <img src={Paid} alt="paid" width={100}/>
}
      </div>
      

      <div className={styles.header}>
        <div className={styles.headerTitle}>Patty Cash VOUCHER</div>
      </div>
     ...
     {data.issues.map((item, index) => (
      ...
      <div className={styles.totalContainer}>
        ...
        <div className={styles.totalAmount}>
          ...
          <div className={styles.totalAmount}>{data.total_paid}</div>
        </div>
      </div>
...
export default ViewPattyCashVoucher;
