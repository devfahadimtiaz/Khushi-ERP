import React from "react";
import styles from "./AddOpeningBalance.module.css";
// Sample data for the table
const AddOpeningBalance = ({ onCancel }) => {
  const EditIcon = () => (
    <svg
      width="20"
      height="20"
      viewBox="0 0 20 20"
      fill="none"
      className={styles.iconSvg}
    >
      <path
        d="M11.25 3.75L4.375 11.25V15H8.125L15.625 7.5M11.25 3.75L13.125 1.875L16.875 5.625L15 7.5L11.25 3.75Z"
        stroke="#4318D1"
        strokeWidth="1.5"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
    </svg>
  );

  const DeleteIcon = () => (
    <svg
      width="20"
      height="20"
      viewBox="0 0 20 20"
      fill="none"
      className={styles.iconSvg}
    >
      <path
        d="M16.875 4.375H3.125M8.125 8.125V13.125M11.875 8.125V13.125M15.625 4.375V16.25C15.625 16.4158 15.5592 16.5747 15.4419 16.6919C15.3247 16.8092 15.1658 16.875 15 16.875H5C4.83424 16.875 4.67527 16.8092 4.55806 16.6919C4.44085 16.5747 4.375 16.4158 4.375 16.25V4.375"
        stroke="#CF2129"
        strokeWidth="1.5"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
    </svg>
  );
  return (
    <div className={styles.container}>
      <div className={styles.contentWrapper}>
        <div className={styles.header}>
          <div className={styles.title}>Add Opening Balance</div>
        </div>
        <div className={styles.contentPanel}>
            <div className={styles.filterHeader}>
              <div className={styles.filterLabel}>Financial Year</div>
              <div className={styles.filterLabel}>Year Status</div>
            </div>
            <div className={styles.filterControls}>
      
            <select className={styles.statusSelect}>
                <option className={styles.selectPlaceholder}>Select Financial Year</option>
                <option className={styles.selectPlaceholder}>2021</option>
                <option className={styles.selectPlaceholder}>2022</option>
                <option className={styles.selectPlaceholder}>2023
                </option>
                </select>
              <select className={styles.statusSelect}>
                <option className={styles.selectPlaceholder}>Year Status</option>
                <option className={styles.selectPlaceholder}>Active</option>
                <option className={styles.selectPlaceholder}>Under Maintenance </option>
                <option className={styles.selectPlaceholder}>Inactive
                </option>
              </select>
            </div>
            <div className={styles.filterHeader}>
              <div className={styles.filterLabel}>Chart of Account</div>
              <div className={styles.filterLabel}>Opening Balance Dr</div>
            </div>
            <div className={styles.filterControls}>
      
            <select className={styles.statusSelect}>
                <option className={styles.selectPlaceholder}>Select Account</option>
                <option className={styles.selectPlaceholder}>Active</option>
                <option className={styles.selectPlaceholder}>Under Maintenance </option>
                <option className={styles.selectPlaceholder}>Inactive
                </option>
              </select>
            <input type="number" id="amount" name="amount" min="0" step="0.01" placeholder="0.00" className={styles.statusSelect}/>
            </div>
            <div className={styles.filterHeader}>
              <div className={styles.filterLabel}>Select Month</div>
              
            </div>
            <div className={styles.filterControls}>
      
            <input type="month"  className={styles.statusSelect}/>
            </div>
            <div className={styles.buttonContainer}>
          <button className={styles.cancelButton} onClick={onCancel}>
            Cancel
          </button>
          <button className={styles.createButton}  onClick={onCancel}>Save</button>
        </div>
          </div>
      </div>
     
    </div>
  );
};

export default AddOpeningBalance;
